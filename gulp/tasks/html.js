...
gulp.task('styles:critical:archive', () => {
  return gulp.src(paths.tempDir  + paths.siteDir + 'pages/index.html')
    .pipe(critical({
      base: paths.tempDir,
      inline: false,
      css: [paths.sassFilesTemp + '/main.css'],
      dimensions: pageDimensions,
      dest: paths.sourceDir + paths.includesFolderName + '/pages-archive.css',
      minify: true,
      extract: false,
...
gulp.task('styles:critical:post', () => {
  return gulp.src(paths.tempDir  + paths.siteDir + 'posts/index.html')
    .pipe(critical({
      base: paths.tempDir,
      inline: false,
      css: [paths.sassFilesTemp + '/main.css'],
      dimensions: pageDimensions,
      dest: paths.sourceDir + paths.includesFolderName + '/post.css',
      minify: true,
      extract: false,
      ignore: ['@font-face','/print/',/url\(/,'.popular-list'] // defer loading of webfonts and background images
    }))
...
gulp.task('styles:critical:home', () => {
  return gulp.src(paths.tempDir  + paths.siteDir + 'index.html')
    .pipe(critical({
      ...
      css: [paths.sassFilesTemp + '/main.css'],
      dimensions: pageDimensions,
      dest: paths.sourceDir + paths.includesFolderName + '/home.css',
      minify: true,
      extract: false,
      ignore: ['@font-face','/print/',/url\(/,'.popular-list'] // defer loading of webfonts and background images
    }))
